...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const anm = apiFetch("/api/announcements")
        .then((r) => r.json())
        .then((d) => {
      ...
      if (user.role === "pengurus_rw") {
        const mon = apiFetch("/api/pengurus-rw/dashboard")
          .then((r) => r.json())
          .then((d) => {
          ...
          <div className="space-y-3">
            {rwData.rts.map((rt) => (
              <Link
                key={rt.rtId}
                href="/dashboard/rw"
                className="card items-center justify-between gap-4 hover:border-emerald-300 block"
              ...
              >
                <div>
                  ...
                  >
                    Lunas {rt.lunas} · Belum {rt.belum} · Rp{" "}
                    {rt.totalPemasukan.toLocaleString("id-ID")}
                  </div>
                </div>
      ...
      <div className="px-4 py-5 max-w-4xl mx-auto">
        ...
        {summary && (
          <div className="grid grid-cols-2 gap-3 mb-5 sm:grid-cols-4">
            ...
            <StatCard
              label="Pemasukan"
              value={`Rp ${Number(summary.totalPemasukan ?? 0).toLocaleString("id-ID")}`}
              green
            />
          </div>
        )}
          ...
          >
            <div>
              ...
              <div className="text-xs" style={{ color: "#92400e" }}>
                Segera tinjau dan setujui
              </div>
            </div>
            <Link
              href="/dashboard/rt/pembayaran"
                    ...
                    {tagihan.slice(0, 8).map((t) => (
                      <tr key={t.id}>
                        ...
                        <td>
                          <span
                            className={
                              STATUS_BADGE[t.status ?? ""] ?? "badge badge-gray"
                            }
                          >
        ...
        <AnnouncementSection
          items={pengumuman}
          manageHref="/dashboard/rt/pengumuman"
        />
      </div>
    );
  }
      ...
      {pendingTagihan.length > 0 && (
        ...
        >
          ...
          <div className="text-xs mt-0.5" style={{ color: "#92400e" }}>
            Tunggu konfirmasi dari Pengurus RT
          </div>
        </div>
      )}
