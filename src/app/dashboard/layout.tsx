...
const ICONS = {
  ...
  wallet: [
    "M21 12V7a2 2 0 00-2-2H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-5",
    "M16 12a2 2 0 104 0 2 2 0 00-4 0",
  ],
  bell: [
  ...
  report: [
    "M9 17H7a2 2 0 01-2-2V7a2 2 0 012-2h10a2 2 0 012 2v3",
    "M9 11h6",
    "M9 15h3",
    "M14 21l2 2 4-4",
  ],
...
const WARGA_TABS: NavItem[] = [
  WARGA_NAV[0],
  WARGA_NAV[1],
  WARGA_NAV[2],
  ACCOUNT_ITEM,
];
const RW_TABS: NavItem[] = [
  RW_NAV[0],
  RW_NAV[1],
  RW_NAV[2],
  RW_NAV[3],
...
function Sidebar({ nav, onLogout }: { nav: NavItem[]; onLogout: () => void }) {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 flex w-sidebar flex-col bg-surface border-r border-border-muted">
      {/* Logo */}
      <div className="flex items-center gap-2 px-4 py-5 border-b border-border-muted">
      ...
      <div
        className="px-3 py-3"
        style={{ borderTop: "1.5px solid var(--border-muted)" }}
      >
        <Link
          href="/dashboard/account"
          className="mb-2 flex items-center gap-2 rounded-lg px-2 py-2 transition-all"
...
function BottomTabBar({ tabs }: { tabs: NavItem[] }) {
  ...
  return (
    <nav
      className="fixed bottom-0 inset-x-0 z-30 flex md:hidden"
      style={{
...
function HamburgerDrawer({
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className="fixed inset-y-0 right-0 z-50 flex flex-col w-64 md:hidden transition-transform duration-200"
        style={{
          background: "var(--surface)",
